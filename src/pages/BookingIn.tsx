...
const BookingIn = () => {
  ...
  const [expandedInvoiceNumber, setExpandedInvoiceNumber] = useState<string | null>(null);
  ...
  const handleToggleUserInvoices = (userEmail: string) => {
    setExpandedUserEmail(expandedUserEmail === userEmail ? null : userEmail);
    setExpandedInvoiceNumber(null);
  };

  const handleToggleInvoiceDetails = async (invoice: GroupedInvoice) => {
    const isExpanding = expandedInvoiceNumber !== invoice.invoice_number;
    setExpandedInvoiceNumber(isExpanding ? invoice.invoice_number : null);

    // Mark as viewed for admins when expanding
    if (isAdmin && !invoice.is_viewed && isExpanding) {
      try {
      ...
      <main className="container py-8">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2 pt-2">
                          <Button
                            ...
                            size="sm"
                            className="flex-1"
                            onClick={() => handleToggleUserInvoices(userGroup.email)}
                          >
                            <span className="md:hidden">{expandedUserEmail === userGroup.email ? 'Hide' : 'View'}</span>
                            <span className="hidden md:inline">{expandedUserEmail === userGroup.email ? 'Hide Invoices' : 'View Invoices'}</span>
                          </Button>
                          <Button
                            variant="destructive"
                            size="sm"
            ...
            {/* Expanded User Invoices Section */}
            {expandedUserEmail && (() => {
              const selectedUser = userInvoices.find(u => u.email === expandedUserEmail);
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {selectedUser.invoices.map((invoice) => {
                          const totalAllocated = invoice.items[0]?.gang_divisions.reduce((sum, m) => sum + m.amount, 0) || 0;
                          const leavesValue = invoice.total_value - totalAllocated;
                          const isExpanded = expandedInvoiceNumber === invoice.invoice_number;
                          
                          return (
                            <Card 
                              key={invoice.invoice_number}
                              className={`${invoice.is_confirmed ? "bg-green-100 dark:bg-green-900/20" : ""}`}
                            >
                              <CardContent className="p-4">
                                <div 
                                  className="flex justify-between items-center cursor-pointer"
                                  onClick={() => handleToggleInvoiceDetails(invoice)}
                                >
                                  ...
                                  <div className="flex items-center gap-4">
                                    <div className="text-right">
                                      <p className="font-bold text-primary">£{invoice.total_value.toFixed(2)}</p>
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          handleToggleInvoiceDetails(invoice);
                                        }}
                                        className="text-sm text-muted-foreground hover:text-primary transition-colors"
                                      >
                                        Leaves: £{leavesValue.toFixed(2)}
                                      </button>
                                    </div>
                                  </div>
                                </div>
                                ...
                                {isExpanded && (
                                  <div className="mt-4 pt-4 border-t space-y-4">
                                    ...
                                    <div>
                                      ...
                                      <div className="space-y-2">
                                        {invoice.items[0]?.gang_divisions.map((member, index) => (
                                          <div key={index} className="flex justify-between items-center p-3 bg-muted/50 rounded-lg">
                                            <div>
                                              <p className="font-medium">{member.member_name}</p>
                                              <p className="text-sm text-muted-foreground capitalize">{member.member_type}</p>
                                            </div>
                                            <p className="font-semibold text-primary">£{member.amount.toFixed(2)}</p>
                                          </div>
                                        ))}
                                        <div className="flex justify-between items-center pt-3 border-t font-bold bg-primary/5 p-3 rounded-lg">
                                          <span>Total Allocated</span>
                                          <span className="text-primary">£{totalAllocated.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between items-center font-bold bg-orange-100 dark:bg-orange-900/20 p-3 rounded-lg">
                                          <span>Leaves (Remaining)</span>
                                          <span className="text-orange-600 dark:text-orange-400">£{leavesValue.toFixed(2)}</span>
                                        </div>
                                      </div>
                                    </div>
                          ...
                          <CardContent className="p-3">
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                ...
                                {invoice.items[0]?.gang_divisions && invoice.items[0].gang_divisions.length > 0 && !isExpanded && (
                                  <div className="mt-2">
                            ...
                            {isExpanded && (
                              <div className="mt-4 pt-4 border-t space-y-4">
                                <div>
                                  <h4 className="font-semibold mb-3 text-sm">Invoice Items</h4>
                                  <div className="space-y-2">
                                    {invoice.items.map((item, index) => (
                                      <div key={index} className="p-3 bg-muted/50 rounded-lg">
                                        {item.is_non_plot ? (
                                          <div className="flex justify-between items-start">
                                            <div>
                                              <p className="font-medium text-sm">Non-Plot Work</p>
                                              <p className="text-xs text-muted-foreground">General brickwork</p>
                                            </div>
                                            <p className="font-bold text-primary text-sm">£{item.booked_value.toFixed(2)}</p>
                                          </div>
                                        ) : (
                                          <>
                                            <div className="flex justify-between items-start mb-2">
                                              <div>
                                                ...
                                                <p className="text-xs text-muted-foreground">{item.plots?.house_types.sites.name}</p>
                                              </div>
                                              <p className="font-bold text-primary text-sm">£{item.booked_value.toFixed(2)}</p>
                                            </div>
                                ...
                                {invoice.notes && (
                                  <div>
                                    ...
                                    <p className="text-xs text-muted-foreground whitespace-pre-wrap bg-muted/50 p-3 rounded-lg">
                                      {invoice.notes}
                                    </p>
                                  </div>
                                )}
                                ...
                                <div>
                                  <h4 className="font-semibold mb-3 text-sm">Gang Division</h4>
                                  <div className="space-y-2">
                                    {invoice.items[0]?.gang_divisions.map((member, index) => (
                                      <div key={index} className="flex justify-between items-center p-3 bg-muted/50 rounded-lg">
                                        <div>
                                          <p className="font-medium text-sm">{member.member_name}</p>
                                          <p className="text-xs text-muted-foreground capitalize">{member.member_type}</p>
                                        </div>
                                        <p className="font-semibold text-primary text-sm">£{member.amount.toFixed(2)}</p>
                                      </div>
                                    ))}
                                    <div className="flex justify-between items-center pt-3 border-t font-bold bg-primary/5 p-3 rounded-lg text-sm">
                                      <span>Total Allocated</span>
                                      <span className="text-primary">£{totalAllocated.toFixed(2)}</span>
                                    </div>
                                    <div className="flex justify-between items-center font-bold bg-orange-100 dark:bg-orange-900/20 p-3 rounded-lg text-sm">
                                      <span>Leaves (Remaining)</span>
                                      <span className="text-orange-600 dark:text-orange-400">£{leavesValue.toFixed(2)}</span>
                                    </div>
                                  </div>

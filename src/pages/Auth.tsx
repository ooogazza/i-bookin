  ...
  return <div className="flex min-h-screen items-center justify-center bg-background">
      <Card className="w-full max-w-md border shadow-sm">
        <CardHeader className="text-center space-y-4 pb-6">
          <img src={logo} alt="I-Bookin Logo" className="h-20 w-20 mx-auto rounded-2xl" />
          <div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">I-Bookin</h1>
            <p className="text-sm font-semibold text-foreground mt-1">Brickwork Invoice Manager</p>
          </div>
        </CardHeader>
        <CardContent className="px-8 pb-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            {isSignUp && <div className="space-y-2">
                ...
                <Input id="fullname" type="text" placeholder="John Smith" value={fullName} onChange={e => setFullName(e.target.value)} required className="h-12" />
              </div>}
            ...
            <Button type="submit" className="w-full h-12 text-base font-medium bg-[#1976D2] hover:bg-[#1565C0] text-white" disabled={isLoading}>
          ...
          <div className="mt-6 text-center space-y-2">
            ...
            {!isSignUp && <div>
                <button type="button" onClick={() => navigate("/reset-password")} className="text-[#1976D2] hover:underline text-sm">
                  Forgot password?
                </button>
              </div>}
          </div>
